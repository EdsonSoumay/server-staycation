...
const expect = chai.expect; //hasil yg kita harapkan
const app = require('../app');
const fs = require('fs');

chai.use(chaiHttp);
describe('API ENDPOINT TESTING', () => {
    it('GET Landing Page', (done) => { //API yg mau kita test
      chai.request(app).get('/api/v1/member/landing-page').end((err, res) => {
        expect(err).to.be.null
        expect(res).to.have.status(200)
        expect(res.body).to.be.an('object') // melihat hasil dalam bentuk object
        expect(res.body).to.have.property('hero') // yg mempunyai properti bernama hero
        expect(res.body.hero).to.have.all.keys('travelers', 'treasures', 'cities') // masing masing key didalam hero
        expect(res.body).to.have.property('mostPicked') // test properti most picked
    ...
    it('GET Detail Page', (done) => {
      ...
      it('POST Booking Page', (done) => {
        ...
        chai.request(app).post('/api/v1/member/booking-page')
          ...
          .field('idItem', dataSample.idItem) //field yang bakal kita kirim
